...
describe SesionesController do
  ...
  before do 
    ...
    Usuario.stub!(:authenticate).with(@login_params[:login], @login_params[:password]).and_return(@user)
  end
  def do_create
    @user.stub!(:rol)
    @rol.stub!(:nombre)
...
describe SesionesController do
  describe "route generation" do
    it "should route the new sesiones action correctly" do
      route_for(:controller => 'sesiones', :action => 'new').should == "/login"
    end 
    it "should route the create sesiones correctly" do
      route_for(:controller => 'sesiones', :action => 'create').should == "/sesion"
    end
    it "should route the destroy sesiones action correctly" do
      route_for(:controller => 'sesiones', :action => 'destroy').should == "/logout"
    end
  end
  
  describe "route recognition" do
    it "should generate params from GET /login correctly" do
      params_from(:get, '/login').should == {:controller => 'sesiones', :action => 'new'}
    end
    it "should generate params from POST /sesion correctly" do
      params_from(:post, '/sesion').should == {:controller => 'sesiones', :action => 'create'}
    end
    it "should generate params from DELETE /sesion correctly" do
      params_from(:delete, '/logout').should == {:controller => 'sesiones', :action => 'destroy'}
    end
  end
  ...
  describe "named routing" do
    ...
    it "should route sesion_path() correctly" do
      sesion_path().should == "/sesion"
    end
    it "should route new_sesion_path() correctly" do
      new_sesion_path().should == "/sesion/new"
    end
  end
