...
{
  warning = "ActiveScaffold Error: Prototype could not be found. Please make sure that your application's layout includes prototype.js (e.g. <%= javascript_include_tag :defaults %>) *before* it includes .js (e.g. <%= _includes %>).";
  alert(warning);
}
...
var ActiveScaffold = {
  ...
  report_500_response: function(_id) {
    messages_container = $(_id).down('td.messages-container');
 ...
 */
function addActiveScaffoldPageToHistory(url, _id) {
  ...
  if (sort || dir || page) dhtmlHistory.add(_id+":"+page+":"+sort+":"+dir, url);
}
